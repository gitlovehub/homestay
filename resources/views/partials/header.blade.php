<header class="sticky top-0 z-50 border-b border-slate-200 bg-white/50 backdrop-blur-lg">
        ...
        <div class="hidden items-center gap-10 md:flex">
            <a href="{{ route('home') }}"
                class="font-medium transition
                    {{ request()->routeIs('home') ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600' }}">
                Trang chủ
            </a>

            <a href="{{ route('homestays.index') }}"
                class="font-medium transition
                    {{ request()->routeIs('homestays.*') ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600' }}">
                Homestay
            </a>

            {{-- Danh mục Desktop --}}
            <div class="group/category static">
                <a href="{{ route('categories.index') }}"
                    class="relative flex items-center gap-2 rounded-lg font-medium transition
                        after:absolute after:inset-x-0 after:top-full after:h-8 after:content-['']
                        {{ request()->routeIs('categories.*') ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600' }}">
                    <span>Danh mục</span>

                    <svg class="h-4 w-4 transition-transform duration-200
                            group-hover/category:rotate-180
                            group-focus-within/category:rotate-180"
                ...
                {{-- Mega menu --}}
                <div
                    class="pointer-events-none invisible absolute inset-x-0 top-full z-50
                    translate-y-2 border-t border-slate-200 bg-white opacity-0 shadow-xl transition duration-200
                    group-hover/category:pointer-events-auto group-hover/category:visible
                    group-hover/category:translate-y-0 group-hover/category:opacity-100
                    group-focus-within/category:pointer-events-auto group-focus-within/category:visible
                    group-focus-within/category:translate-y-0 group-focus-within/category:opacity-100">
                    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                        <div class="mb-5 flex items-center justify-between gap-4">
                            <div>
                                <h2 class="text-lg font-bold text-slate-900">Danh mục Homestay</h2>
                                <p class="mt-1 text-sm text-slate-500">
                                    Lựa chọn loại hình lưu trú phù hợp với chuyến đi của bạn.
                                </p>
                            </div>
                            <a href="{{ route('categories.index') }}"
                                class="shrink-0 text-sm font-semibold text-blue-600 transition hover:translate-x-1 hover:text-blue-700">
                                Xem tất cả →
                            </a>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                            @forelse ($navCategories as $category)
                                <a href="{{ route('categories.show', $category->slug) }}"
                                    ...
                                    <svg class="h-4 w-4 shrink-0 text-slate-400 transition group-hover:translate-x-1 group-hover:text-blue-600"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @empty
                                <div
                                    ...
                                    <p class="text-sm text-slate-500">Chưa có danh mục Homestay.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
            ...
            <a href="{{ route('about') }}"
                class="font-medium transition
                    {{ request()->routeIs('about') ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600' }}">
                Giới thiệu
            </a>

            <a href="{{ route('contact') }}"
                class="font-medium transition
                    {{ request()->routeIs('contact') ? 'text-blue-600' : 'text-slate-600 hover:text-blue-600' }}">
                Liên hệ
            </a>
        </div>
            ...
            @auth
                <div class="relative" id="user-dropdown">
                    <button type="button" id="user-menu-button"
                        class="flex cursor-pointer items-center gap-2.5 focus:outline-none" aria-expanded="false"
                        aria-haspopup="true">
                        @if (auth()->user()->avatar)
                        ...
                        <svg id="user-chevron" class="hidden h-4 w-4 text-slate-400 transition lg:block" fill="none"
                    ...
                    <div id="user-menu"
                        class="absolute right-0 z-50 mt-2 hidden w-64 origin-top-right overflow-hidden rounded-2xl border border-slate-100 bg-white pt-2 pb-0 font-semibold shadow-xl shadow-slate-200/60 ring-1 ring-black/5">
                        ...
                        <div class="border-t border-slate-100">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="group flex w-full cursor-pointer items-center gap-3 px-4 py-2.5 text-sm text-red-600 transition hover:bg-red-50">
                                    <span
...
<div id="mobile-overlay"
    class="pointer-events-none fixed inset-0 z-60 opacity-0 backdrop-blur-sm transition-opacity duration-300 md:hidden">
</div>
...
<div id="mobile-sidebar"
    ...
    <div class="flex h-[calc(100%-65px)] flex-col overflow-y-auto">
        <div class="space-y-1 px-3 py-4">
            ...
            {{-- Danh mục Mobile --}}
            <div x-data="{ categoryOpen: {{ request()->routeIs('categories.*') ? 'true' : 'false' }} }" class="overflow-hidden rounded-lg">
                <button type="button" @click="categoryOpen = !categoryOpen" :aria-expanded="categoryOpen"
                    ...
                    :class="categoryOpen || {{ request()->routeIs('categories.*') ? 'true' : 'false' }} ?
                        'bg-blue-50 text-blue-600' :
                        'text-slate-600'">
                    <span>Danh mục</span>
                ...
                <div x-show="categoryOpen" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    @empty
                        <p class="px-3 py-2.5 text-sm text-slate-400">Chưa có danh mục</p>
                    @endforelse
                </div>
            </div>
        ...
        <div class="mt-auto border-t border-slate-100 px-3 py-4">
            @auth
                ...
                <a href="{{ route('profile.edit') }}"
                    class="mb-1 flex items-center gap-3 rounded-lg px-3 py-2.5 text-base font-medium text-slate-600 transition hover:bg-slate-50 hover:text-blue-600">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                    </svg>
                ...
                <form method="POST" action="{{ route('logout') }}" class="mt-2">
                    @csrf
                    <button type="submit"
                        class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-red-700">
            ...
            @else
                <a href="{{ route('login') }}"
                    class="mb-2 block rounded-lg px-3 py-2.5 text-center text-base font-medium text-slate-700 transition hover:bg-slate-50">
                    Đăng nhập
                </a>
                <a href="{{ route('register') }}"
