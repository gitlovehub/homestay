...
>
    ...
    <button type="button" @click="mobileFilterOpen = true" :aria-expanded="mobileFilterOpen"
        ...
        <div x-show="mobileFilterOpen" x-transition:enter="transform transition ease-out duration-300"
            ...
            <div class="flex-1 overflow-y-auto px-5 py-4">
                <form method="GET" action="{{ $filterAction }}" class="space-y-4" id="{{ $formId }}">
                    ...
                    <div>
                        <label for="{{ $idPrefix }}-room-type" class="text-sm font-semibold text-slate-700">Loại phòng</label>
                        <select id="{{ $idPrefix }}-room-type" name="room_type"
                    ...
                    <div>
                        <label for="{{ $idPrefix }}-rating" class="text-sm font-semibold text-slate-700">Đánh giá</label>
                        <select id="{{ $idPrefix }}-rating" name="rating"
                            class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                            <option value="">Tất cả</option>
